...
let centre_2d [n][m] (arr: [n][m]c32): [n][m]c32 =
  ...
  in map (\(i,r) -> map (\(j,x) -> f i j x) (zip (map i32.i64 (iota m)) r)) (zip (map i32.i64 (iota n)) arr)
...
let transform [n][m] (cutoff: i32) (arr: [n][m]u8) =
  ...
  let arr_filt = map (\(i,r) -> map (\(j,x) -> zap i j x) (zip (map i32.i64 (iota m)) r))
                     (zip (map i32.i64 (iota n)) arr_freq)
